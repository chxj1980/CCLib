...
bool CSpreadSheet::Commit()
{
	if (m_bExcel) // If file is an Excel spreadsheet
	{
		...
		if (m_bAppend)
		{
			...
			m_stempString= L"[" + m_sSheetName + L"$A1:IV65536]";
			...
			{
				m_stempSql = m_stempSql + L"[" + m_aFieldNames.GetAt(j) +L"]" + L" char(255), ";
			}
			m_stempSql.Delete(m_stempSql.GetLength()-2, 2);
			m_stempSql += ")";
		}
...
void CSpreadSheet::GetExcelDriver()
{
	...
	if(!SQLGetInstalledDrivers(CharToLpwStr(szBuf),cbBufMax,& cbBufOut))
	{
		m_sExcelDriver = "";
	}
